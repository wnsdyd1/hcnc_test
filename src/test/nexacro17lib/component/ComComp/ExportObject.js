...
if (!nexacro.ExcelExportObject) {
	...
	_pExcelExport.exportData = function (argsParam, argsDsParam, nOrgValueType, bFileDownload) {
		...
		if (this.exportfilepassword) {
			this._file_pw = this.exportfilepassword;
		}
		...
		for (i = 0; i < g_len; i++) {
			if (grid_items[i].source._getGridRowCount() == 0) {
				grid_items[i]._is_change_rowcount = true;
				grid_items[i].source._rowcount = 1;
			}
		...
		for (i = 0; i < g_len; i++) {
			if (grid_items[i]._is_change_rowcount) {
				grid_items[i]._is_change_rowcount = false;
				grid_items[i].source._rowcount = 0;
			}
		}
	...
	_pExportItem._is_change_rowcount = false;
		...
		switch (cell_type) {
			...
			case "imagecontrol":
				if (this.exportimage.toLowerCase() == "image") {
					displaytype += "image";
					var _imgstretch = cell._getAttrValue(cell.imagestretch, rowidx);
					if (_imgstretch) {
				...
				for (j = 0; j < bodycntcell; j++) {
					...
					if (display_type == "imagecontrol") {
						...
						if (!maskeditpostfixtext_table) {
							var maskeditpostfixtext_table = new nexacro.Collection();
						}
						var maskeditpostfixtext = maskeditpostfixtext_table.getItem("col" + j);
						if (maskeditpostfixtext == undefined) {
							maskeditpostfixtext = cell._getAttrValue(cell.maskeditpostfixtext, k);
							maskeditpostfixtext_table.setItem("col" + j, maskeditpostfixtext ? maskeditpostfixtext : "none");
						}
