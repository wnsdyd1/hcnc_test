...
if (!nexacro.Form) {
	...
	_pFormBase._on_bubble_close = function (event_bubbles, fire_comp, refer_comp) {
		...
		if (parent && parent._on_bubble_close) {
	...
	_pForm.cancelTransaction = function (id) {
		...
		this._stopTransaction(true, datalist);
	};
	...
	_pForm._stopTransaction = function (is_cancel, willCancelDatalist) {
		if (!this._load_manager) {
			return -1;
		}

		var datalist = (is_cancel === true) ? willCancelDatalist : this._load_manager.dataList;
