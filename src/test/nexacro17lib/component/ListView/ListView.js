...
if (!nexacro.ListView) {
	...
	_pListView._enable_redraw_history = {
	};
	...
	_pListView._on_basic_onselectedchange = function () {
		this._updateBindItemInfo();
	};
	...
	_pListView._onChangeSelect = function (obj, type, oldvalue, newvalue) {
		...
		this._on_basic_onselectedchange();
	...
	_pListView.on_apply_enableredraw = function () {
		...
		if (this._enable_redraw_history.updatebinditeminfo) {
			this._updateBindItemInfo();
		}
		else if (this._enable_redraw_history.updateitem) {
			var props = this._enable_redraw_history.updateitem;
			var props_len = props.length;

			for (var i = 0; i < props_len; i++) {
				this._updateItem(props[i][0], props[i][1], props[i][2], props[i][3]);
			}
		}

		if (this._enable_redraw_history.recalclayout) {
		...
		if (Object.keys(this._enable_redraw_history).length > 0) {
			this._recreateItems();
		}
	};
	...
	_pListView.hideDetailBand = function (rowindex) {
		...
		if (slot) {
			...
			if (this.bandexpandtype == "expand" || this.bandexpandtype == "accordion") {
				if (this.enableredraw && nexacro._isArray(items)) {
					this._updateItemScrollInfo("itemshow", true);
				}
			}
	...
	_pListView._updateBindItemInfo = function () {
		if (!this.enableredraw) {
			this._enable_redraw_history.updatebinditeminfo = true;
			return;
		}
