...
if (!nexacro.ComplexComponent) {
		...
		if (panel && panel._panel_subgroup && index !== undefined) {
			...
			if (recalc != false) {
				if (!this.enableredraw) {
					this._enable_redraw_history.recalclayout = true;
					return;
				}
	...
	_pComplexComponent._updateItem = function (item, index, info, bandseq) {
		if (!this.enableredraw) {
			if (!this._enable_redraw_history.updateitem) {
				this._enable_redraw_history.updateitem = [];
			}

			this._enable_redraw_history.updateitem.push([item, index, info, bandseq]);
			return;
		}
