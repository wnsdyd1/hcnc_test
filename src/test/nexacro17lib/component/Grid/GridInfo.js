...
if (!nexacro.GridFormat) {
	...
	_pGridFormat._moveColumn = function (fromcol, tocol, fromcolspan, tocolspan, cellmovingtype) {
		...
		var from_orgleft = from_col_info.orgleft;
		var from_orgright = from_col_info.orgright;
		...
		if (fromcol > tocol) {
			for (i = tocol; i < fromcol; i++) {
				...
				this._cols[i].orgleft = this._cols[i + 1].orgleft;
				this._cols[i].right = this._cols[i + 1].right;
				this._cols[i].orgright = this._cols[i + 1].orgright;
			}
		}
		else {
			for (i = tocol; i > fromcol; i--) {
		...
		this._cols[fromcol].orgleft = from_orgleft;
		this._cols[fromcol].right = from_right;
		this._cols[fromcol].orgright = from_orgright;
