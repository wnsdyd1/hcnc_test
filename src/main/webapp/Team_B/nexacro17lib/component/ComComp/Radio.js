...
if (!nexacro.Radio) {
		...
		if (keycode == nexacro.Event.KEY_SPACE) {
			if (!this.readonly) {
				if (accIdx > -1) {
					...
					var radioitem = items[accIdx];

					var ret = this.on_fire_canitemchange(this, pre_index, pre_text, pre_value, accIdx, radioitem.text, radioitem.value);
					if (ret) {
						items[accIdx]._changeUserStatus("focus", true);
						this.set_index(accIdx);
						if (accIdx != pre_index) {
							this.on_fire_onitemchanged(this, pre_index, pre_text, pre_value, accIdx, radioitem.text, radioitem.value);
						}
					}
				}
			}
	...
	_pRadio._on_radio_onkeydown = function (obj, evt) {
		var E = nexacro.Event;
