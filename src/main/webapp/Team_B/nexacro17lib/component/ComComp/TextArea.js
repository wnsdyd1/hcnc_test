...
if (!nexacro.TextArea) {
	...
	_pTextArea._onRecalcScrollSize = function () {
		...
		if (control_elem && input_elem) {
			...
			max_height = input_elem.getElementScrollHeight(noflush);
		...
		if (nexacro._enableaccessibility) {
			if (!this._onlydisplay) {
				if (keycode == nexacro.Event.KEY_UP) {
					...
					if (!this.enable || this.readonly || (elem && elem.isFirstCaretLine()) || !elem) {
						want_arrow = false;
					}
				}
				else if (keycode == nexacro.Event.KEY_DOWN) {
					elem = this._input_element;
					if (!this.enable || this.readonly || (elem && elem.isLastCaretLine()) || !elem) {
						want_arrow = false;
					}
				}
			}
