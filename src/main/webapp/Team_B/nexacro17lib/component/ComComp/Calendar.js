...
if (!nexacro.Calendar) {
	...
	_pCalendar._on_edit_oneditclick = function (obj, e) {
		...
		var _last_comp = this._find_lastFocused();
		if (nexacro._OS == "iOS" && parseFloat(nexacro._OSVersion) >= 15 && _last_comp instanceof nexacro.Calendar && nexacro._is_openSystemCalendar) {
			nexacro._is_openSystemCalendar = false;
			return;
		}
					...
					if (result == null) {
						...
						if (bPaste) {
							...
							if (input_pos < 0 || maskobj.isFilterChar(insert_text, input_pos)) {
								input_elem.updateElementText(this.text, input_pos);
							}
							else {
								input_pos = maskobj.findNearEditablePos(result.pos, 1);
								if (nexacro._Browser == "Runtime" && this.autoselect && (this.text == result.text)) {
