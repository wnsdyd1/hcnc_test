...
if (!nexacro.Grid) {
	...
	_pGridCell._updateAll = function (status, onlycontents, nochk_updatearea) {
		if (this.__update(status, onlycontents, nochk_updatearea)) {
			...
			if (control_elem) {
				...
				if (!onlycontents) {
					this._control_element.setElementBorderNone(remove_l, remove_t, remove_r, remove_b, nexacro._toBoolean(grid.rtl));
				}
		...
		if (dir > 0) {
			...
			if (is_updatecontents) {
				...
				if (hide_rows.length == 0) {
					if (org_row && org_row._rowidx !== undefined && band._update_rows != hide_rows) {
						var rowidx = org_row._rowidx;
						var cellinfos = grid._getCellinfos(rowidx);
						if (cellinfos) {
							if (cellinfos.length == 1) {
								var cellinfo = cellinfos[0];
								var level = cellinfo._getTreeLevel(rowidx);
								var state = cellinfo._getTreeState(rowidx);
							}
							else {
								for (var i = 0; i < cellinfos.length; i++) {
									var cellinfo = cellinfos[i];
									var displaytype = cellinfo._getDisplaytype(rowidx);
	...
	_pGridBand.on_change_containerRect = function (width, height) {
		...
		if (grid._is_changingRect) {
			if (grid._colautofit) {
				...
				else if (grid._isLazyAutoVscroll() && (grid.autosizingtype == "row" || grid.autosizingtype == "both")) {
					grid._resetRowSizeList();
					grid._resetColSizeList();
				}
			}
	...
	_pGridBand._callback_update_rect = function (no_ani) {
		...
		if (this._isBody) {
			...
			if (grid._needUpdateExtinner(true) || (grid._isLazyAutoVscroll() && (grid.autosizingtype == "row" || grid.autosizingtype == "both"))) {
	...
	_pGrid._adjustGridScrollRows_callback_onscroll_after2 = function (pos) {
		...
		if (this._use_enhance_performance_scrolling) {
			...
			while (bcontinue && (timestamp - lasttime + singleframe_duration) <= this._scroll_frame_interval) {
				bcontinue = body._matrix._adjustScrollRows2(pos, true, framecnt);
				body._on_refresh_rows(true, undefined, true);
			...
			if (prevpos > postvpos || (prevpos < postvpos && this._vscrollmng._orgmax != prevpos)) {
				if (evttype == "linedown") {
					while (true) {
						...
						if (prev_scrollbar_pos == this._vscrollmng._pos) {
							postvpos++;
							continue;
						}
				...
				if (this._use_eachscroll || this._isLazyAutoVscroll()) {
					this._startApplyTimer("v", (nexacro._Browser == "IE") ? 1000 : 500);
				}
			}
		}
		else if (hmove > vmove) {
	...
	_pGrid._adjustRtlCanvasX = function (obj, canvasX) {
		if (nexacro._toBoolean(this.rtl)) {
	...
	_pGrid._on_grid_lbuttondown = function (cellobj, band, ctrlkey, shiftkey, no_select, metakey) {
		...
		if ((this._isAreaSelect() || this._isMultiSelect()) && !nexacro._isTouchInteraction && this.selectchangetype == "down") {
			...
			if (nexacro._Browser == "Runtime") {
				...
				while (parent) {
					if (parent._type_name == "Form") {
						...
						parent_scroll_top = parent._control_element.scroll_top - (parent._control_element.scroll_top > 0 ? this._adjust_top : 0);
						break;
					}
					parent = parent.parent;
				}
			...
			if (_reason == 1 || (_reason == 0 && e.progressload)) {
				...
				if (this.autosizingtype != "none") {
					if (this._async_create == true) {
						...
						if (_reason == 2 && this._isDownActionKeyMouse() && this._userRowposChange) {
			...
			if (nexacro._enableaccessibility) {
				...
				if (self_flag == false) {
					if (evt_name == "lbuttondown" && refer_new_focus && refer_new_focus._type_name == "GridCellControl") {
						...
						if (nexacro._Browser == "IE" || nexacro._Browser == "Opera" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge") || nexacro._BrowserType == "Runtime") {
							this._onceTime_focus = true;
						}
	...
	_pGrid._resetSelect = function (row, cell, col, subrow, pvt) {
		...
		if (this.getElement()) {
			...
			this._ChangeSelect(cell, col, row, subrow, pvt, true, bcell, bcol, brow, bsubrow, bpvt, "body", undefined, true);
			this._refreshHead(true);
			this._refreshSumm(true);
		}
	};

	_pGrid._ChangeSelect = function (cell, col, row, subrow, pvt, bDataset, oldcell, oldcol, oldrow, oldsubrow, oldpvt, bandstr, evt_kind, is_reset) {
		...
		if (selectmode == "ctrl") {
			if (this.selecttype == "multirow") {
				if (!this._isIncludeSelectpos(0, row) || (evt_kind == "func_selectmultirow")) {
					ctrlkey_change = true;
				}
			}
		...
		if (selectmode == "none") {
			...
			if (this.selecttype == "multiarea" || this.selecttype == "multirow") {
		...
		if (b_draw) {
			if (is_reset && this.autoenter == "select") {
				this._hideEditor(true);
			}
		...
		if (format._bodyrows) {
			...
			if (!noauto && this._binddataset && this._bodyAutoSize && (this._autoSizeRowProc || this.autosizingtype == "row" || this.autosizingtype == "both")) {
				...
				if (lazy_rowsize) {
					...
					endpos = Math.min(this._getDataRow(endpos > this.rowcount - 1 ? this.rowcount - 1 : endpos), this._rowcount);
				}
	...
	_pGrid._isLazyAutoVscroll = function () {
		return this._lazy_calculate && (this.autosizingtype == "row" || this.autosizingtype == "both") && this.extendsizetype == "row";
	};
	...
	_pGrid._getMinRowTextHeight = function () {
		...
		return nexacro._getTextSize(".", font)[1];
	};
	...
	_pGrid._resetRowSizeListBodyRange = function (top_datarow, end_datarow, retn_rows, retn_subrows) {
		...
		if (format._bodyrows) {
			...
			var _rowSizeListSub = this._rowSizeListSub, _rowSizeList = this._rowSizeList;
			...
			if (!noauto && this._binddataset && this._bodyAutoSize && (this._autoSizeRowProc || this.autosizingtype == "row" || this.autosizingtype == "both")) {
				for (i = top_datarow; i <= end_datarow; i++) {
					if (this._lazy_load_rowsize[i] == undefined) {
						h = 0;
						for (j = 0; j < rowsLen; j++) {
							height = this._getMaxSubRowSize(i, j);
							_rowSizeListSub[i *  rowsLen + j] = height;
	...
	_pGrid._getTempCell = function (cellidx, rowidx, parent_cellinfo, autosizing, prop) {
		...
		if (band_control) {
			...
			if (row_control) {
				...
				var pre_userstatus = cell_control._userstatus;
				if (autosizing) {
				...
				if (cell_control._userstatus != pre_userstatus) {
					cell_control._is_userstatuschanged = true;
				}
	...
	_pGrid._getCellStyleInfo = function (cellidx, prop, datarow, selected, parent_cellinfo, autosizing, bexport) {
		...
		if (cell._is_userstatuschanged == true) {
			cell._is_userstatuschanged = undefined;
			use_flush = true;
		}
	...
	_pGrid._getCellRowTextSize = function (cellinfo, rowidx, text, parent_cellinfo, only_normal, merge_width) {
		...
		if (displayType == "imagecontrol") {
			...
			if (usewordwrap && (this._autoSizeRowProc || this._rowSizeEx) && word != null && word != "none" && word != false && word != "false") {
				...
				width -= ctrl_width;
				size = nexacro._getTextSize(text, font, true, width, word, wordspacing, letterspacing, usedecorate, halign, valign, nexacro._toBoolean(this._rtl));
			}
			else {
				size = nexacro._getTextSize(text, font, true, undefined, undefined, wordspacing, letterspacing, usedecorate, undefined, undefined, nexacro._toBoolean(this._rtl));
			}
	...
	_pGrid._applyResizer = function () {
		...
		if (this.cellsizingtype == "col" || this.cellsizingtype == "both") {
			...
			resizer_colctrl.set_rtl(nexacro._toBoolean(this.rtl));
