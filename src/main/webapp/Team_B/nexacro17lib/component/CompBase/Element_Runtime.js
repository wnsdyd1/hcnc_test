...
if (nexacro._Browser == "Runtime") {
	...
	if (!nexacro.Element) {
		...
		_pInputElement.updateElementText = function (value, caretpos, beginpos) {
			...
			if (this.autoselect && (beginpos != null || beginpos != undefined)) {
		...
		_pInputElement._updateInputValue = function (value, fireevent, selectionStart, selectionEnd) {
			...
			if (handle) {
				...
				if (selectionStart != null && selectionStart > -1) {
					...
					if (this._is_focused) {
						if (this.autoselect && checkAutoSelc && (selectionEnd != -99)) {
		...
		_pInputElement._on_sys_compositionend = function () {
			...
			if (handle) {
				if (this._inputtype == "") {
					if (this._composer.status == nexacro._CompositionState.START || 
						this._composer.status == nexacro._CompositionState.COMPOSING) {
						var pos = this.getElementCaretPos();
						this._composer.setStatus(nexacro._CompositionState.END, pos.end);
					}
				}
		...
		_pTextAreaElement.getElementScrollWidth = function (noflush) {
			var handle = this.handle;
			if (handle) {
				if (!this._is_focused && this.value == null && this.displaynulltext) {
					return this.width;
				...
				return nexacro.__getInputElementHandleScrollWidth(handle, noflush);
			}
			return 0;
		};

		_pTextAreaElement.getElementScrollHeight = function (noflush) {
			var handle = this.handle;
			if (handle) {
				if (!this._is_focused && this.value == null && this.displaynulltext) {
					return this.height;
				...
				return nexacro.__getInputElementHandleScrollHeight(handle, noflush);
			}
			return 0;
		};
