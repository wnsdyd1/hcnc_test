...
if (nexacro._Browser != "Runtime" && !nexacro.Element) {
	...
	_pInputElement.createCommand = function () {
		...
		if (owner_elem && !this._created) {
			...
			if (!nexacro._isNull(this.value) || this.value === "") {
				...
				if (this.inputtype == "password") {
					str += "value ='" + nexacro._encodeXml(this.displaynulltext) + "' ";
					handle_attr += nexacro.__getHTMLAttr_InputType("password");
				}
				else {
					str += "value ='" + nexacro._encodeXml(this.displaynulltext) + "' ";
					handle_attr += nexacro.__getHTMLAttr_InputType("text");
		...
		if (input_handle) {
			...
			if (!this._is_sys_focused) {
				...
				if (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge") {
					...
					if (nexacro._OS == "iOS" && pThis.autoselect && pThis.inputtype != "date") {
		...
		if (input_handle) {
			...
			if (this._is_focused) {
				if (this.displaynulltext) {
					...
					else if (pThis.value == null && pThis.displaynulltext != "" && !pThis._is_focused) {
						nexacro.__changeInputDOMNodeType(input_handle, "text");
					}
				}
	...
	_pInputElement.on_sys_keydown_before_process = function (evt) {
		...
		if ((nexacro._isTouchInteraction && nexacro._SupportTouch) || nexacro._isMobile()) {
			var _form = comp.parent, stepselector = null;
			while (_form) {
				if (_form.stepselector) {
					stepselector = _form.stepselector;
					break;
				}
				_form = _form.parent;
			}
			...
			if (stepselector) {
				...
				if (stepselector.stepindex != steppos) {
					stepselector.set_stepindex(steppos);
				}
			}
		}
					...
					if (focuspath_index == -1) {
						...
						if (old_focused_comp) {
							...
							if (nexacro._Browser == "Chrome" || nexacro._Browser == "Opera" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "WebKit") || nexacro._OS == "iOS") {
								comp.on_apply_value(comp.value, pThis._is_invalid_value);
							}
	...
	__pWebBrowserPluginElement.create = function (win) {
		...
		if (owner_elem && owner_elem.handle && !this.handle) {
			...
			this._createInternalPrevElementHandle(win, _doc, owner_elem, handle);

			this.handle = this.dest_handle = handle;
			nexacro.__appendDOMNode(owner_elem.dest_handle, handle);
			nexacro._observeSysEvent(handle, "load", "onload", _iframe_eventhandler_onload);

			this._createInternalNextElementHandle(win, _doc, owner_elem, handle);
	...
	__pWebBrowserPluginElement._createInternalPrevElementHandle = function (win, _doc, owner_elem, handle) {
	...
	__pWebBrowserPluginElement._createInternalNextElementHandle = function (win, _doc, owner_elem, handle) {
		if (nexacro._enableaccessibility) {
	...
	__pWebBrowserPluginElement._createInternalFrameElementHandle = function (_doc, left, top, width, height, params) {
		...
		nexacro.__setDOMNode_Title(handle, this.parent_elem.accessibilitylabel ? this.parent_elem.accessibilitylabel : this.parent_elem.linkedcontrol.id);
	...
	_pGridScrollableControlElement.setElementHScrollPos = function (hpos) {
		if (this.parent.enableredraw == false) {
	...
	__pGridBandContainerElement._changeInnerElement = function (top, row_handle) {
		...
		if (prev_handle && (prev_handle != next_handle)) {
			...
			var elem = row_handle._linked_element;
			var hpos = this._findScrollbarControl(elem).pos;
			elem._client_body_element._setElementHScrollPos(hpos);
			elem.scroll_left = hpos;
		}
				...
				if (handle) {
					...
					if (_inner_handle && _ext_inners_len > 0) {
						...
						if (vpos <= scroll_max_arr[0]) {
							nexacro.__setDOMStyle_Visible(_inner_handle.style, true);
						}
						else {
							nexacro.__setDOMStyle_Visible(_inner_handle.style, false);
						}
						...
						if (dir > 0) {
							for (i = 0, n = scroll_max_arr.length - 1; i < n; i++) {
								...
								if (vpos >= scroll_max_arr[i]) {
									if (extinner_handle) {
										nexacro.__setDOMStyle_Visible(extinner_handle.style, false);
									}
							...
							if (extinner_handle) {
								...
								nexacro.__setDOMStyle_Visible(extinner_handle.style, true);

								if (nextinner_handle) {
									nexacro.__setDOMStyle_Translate(nextinner_handle.style, 0, extinner_height);
									nexacro.__setDOMStyle_Visible(nextinner_handle.style, true);
									i++;
								}

								for (; i < _ext_inners_len; i++) {
									nexacro.__setDOMStyle_Visible(_ext_inners[i].style, false);
								}
								...
								for (i = 0; i < _ext_inners_len; i++) {
									if (i == 0) {
										...
										nexacro.__setDOMStyle_Visible(_ext_inners[i].style, true);
									}
									else {
										nexacro.__setDOMStyle_Visible(_ext_inners[i].style, false);
									}
	...
	nexacro.GridRowControlElement = function (parent_elem, left_gap) {
		...
		this._client_body_element._no_focus_scroll = true;
	...
	_pGridRowControlElement.setElementHScrollPos = function (hpos) {
		if (this.parent._grid.enableredraw == false) {
	...
	_pCellControlElement.setElementBorderNone = function (borderleft, bordertop, borderright, borderbottom, rootcomp_rtl) {
		if (borderleft != this.borderLeftNone || bordertop != this.borderTopNone || borderright != this.borderRightNone || borderbottom != this.borderBottomNone) {
			this.borderLeftNone = borderleft;
			this.borderTopNone = bordertop;
			this.borderRightNone = borderright;
			this.borderBottomNone = borderbottom;
			this._border_rootcomp_use_rtl = rootcomp_rtl;

			this._refreshBorderNone();
		}
	};

	_pCellControlElement._get_rtl = function () {
		var rtl = this._border_rootcomp_use_rtl != undefined ? this._border_rootcomp_use_rtl : this.rtl;
		return rtl;
	};
}
