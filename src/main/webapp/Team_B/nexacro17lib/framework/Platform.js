		...
		if (x === undefined && y === undefined) {
			...
			while (tmp) {
				if (!tmp._is_component) {
					...
					if (tmp instanceof nexacro.ModalOverlayElement) {
						if (last_obj == tmp.parent) {
							tmp = null;
							break;
						}
						if (tmp && tmp.parent) {
			...
			if (nexacro._Browser == "Runtime" && (nexacro._SystemType.toLowerCase() == "win32" || nexacro._SystemType.toLowerCase() == "win64")) {
				...
				if (this.frame && this.frame._is_click_openstatus === false) {
					if (this.frame._state_openstatus == 2) {
						if (wparam === 0) {
							var modal_frame = this._getLastModalFrame();
							if (modal_frame) {
								var left = modal_frame._adjust_left;
								var top = modal_frame._adjust_top;
								var width = modal_frame._adjust_width;
								var height = modal_frame._adjust_height;

								var _window = modal_frame._getWindow();
								var after_align_pos = modal_frame._getOpenAlignPos(_window, left, top, width, height);
								if (after_align_pos) {
									modal_frame._move(after_align_pos.left, after_align_pos.top, width, height);
								}
							}
