...
if (nexacro._Browser != "Runtime" && !nexacro._init_platform_HTML5) {
	...
	if (nexacro._Browser == "IE" || nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge") {
		...
		nexacro._createSysEvent_ForwardFuncs = function (_cur_win) {
			...
			_cur_win._syshandler_onresize_forward = function (evt) {
				if (nexacro._OS == "Android" && screen) {
					var prev_screen_width = this._cur_screen_width;
					var cur_screen_width = screen.width;
					var cur_screen_height = screen.height;
					if (cur_screen_width && prev_screen_width) {
						if (cur_screen_width != prev_screen_width && cur_screen_height != prev_screen_width) {
							var savedscreeninfo = nexacro._getLocalStorage("screeninfo", 2);
							if (savedscreeninfo) {
								var selectscreen = nexacro._getScreen(JSON.parse(savedscreeninfo));
								if (selectscreen) {
					...
					this._cur_screen_width = cur_screen_width;
				}
				return nexacro._syshandler_onresize(_cur_win.nexacro_HTMLSysEvent, evt);
			};
			_cur_win._syshandler_onorientationchange_forward = function (evt) {
	...
	nexacro._getLocalStorageKey = function (type, global) {
		...
		if (global) {
			...
			if (type < 2) {
				...
				localstoragekey = stat + nexacro._uniquestoragevalue + env.key + projectpath;
			}
		}

		switch (type) {
			case 1:
				localstoragekey = localstoragekey + "user";
				break;
			case 2:
				localstoragekey = stat + nexacro._uniquestoragevalue + projectpath + "engine";
				break;
			case 3:
				localstoragekey = localstoragekey + "envvar";
				break;
			case 4:
				localstoragekey = localstoragekey + "envcookie";
				break;
			case 5:
				localstoragekey = localstoragekey + "envhttp";
				break;
			case 6:
				localstoragekey = localstoragekey + "envsecurecookie";
				break;
		}
		if (type > 1) {
	...
	nexacro.__setViewportScale = function () {
		...
		else if (nexacro._BrowserExtra == "NaverBrowser" || nexacro._BrowserExtra == "MiuiBrowser" || (nexacro._OS == "iOS" && (nexacro._Browser == "MobileSafari" || nexacro._Browser == "Chrome"))) {
	...
	nexacro._searchDeviceExceptionTable = function () {
		...
		for (var i = 0; i < len; i++) {
			...
			if (navigator.userAgentData) {
				if (nexacro._accurateModel.indexOf(table[i].model) >= 0) {
					return table[i];
				}
			}
			else {
